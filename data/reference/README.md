...
経緯：過去に二次資料由来のコード誤記が判明したため、総務省の全国地方公共団体コードのみを一次資料とする本ルールを策定。
...
2. `data/reference/` に `soumu-jichitai-codes-{YYYY-MM-DD}.xls` として保存
...
- **partial（部分カバレッジ）**：北海道〜千葉県袖ケ浦市までの抜粋を手動 curate
