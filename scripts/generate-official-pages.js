...
const PROVENANCE_HOST_EXTRA = new Set([
  ...
  // 2026-09-07 追加: 独自ドメインで公式サイトを運営する4自治体。GOVERNANCE §E-2 の基準で
  //   オーナーが目視確認のうえ許可（法人番号が無い場合も、例規集の自ドメイン掲載・選管告示の収録・
  //   県公式サイトからのリンク等を実測で確認できれば公式出典とみなす）。
  "www.higashikagawa.jp",        // 東かがわ市（香川県）— 法人番号8000020372072・〒769-2792 東かがわ市湊1847番地1・
                                 //   Copyright (c) Higashikagawa City・市例規集/市議会/市長の部屋を自ドメインに掲載
  "www.city-yanai.jp",           // 柳井市（山口県）— 法人番号4000020352128・〒742-8714 柳井市南町一丁目10番2号・
                                 //   Copyright Yanai City・山口県の消防防災リンク集から当ドメインへリンク
  "www.kouratown.jp",            // 甲良町（滋賀県）— 法人番号の掲載は無い。〒522-0244 犬上郡甲良町在士353-1・
                                 //   Copyright Koura Town・例規集を自ドメインで掲載（内容現在 令和8年3月31日）・
                                 //   甲良町選挙管理委員会規程（選管告示第34号）等の選管例規を収録・
                                 //   滋賀県「県内の市町一覧」から当ドメインへリンク
  "www.townkamiita.jp",          // 上板町（徳島県）— 法人番号8000020364053・〒771-1392 板野郡上板町七條字経塚42番地・
                                 //   copyright Kamiita-Town・徳島県「徳島県の市町村一覧」から当ドメインへリンク
  ...
  // 2026-09-12 追加: 長野県の4村。いずれも自治体系ドメインが使えず、村自身の広報誌・公文書を
  //   当該ドメインで配信していることを実測で確認した（詳細は registry/provenance-host-watch.json）。
  "www.minamiaiki.jp",           // 南相木村（長野県）〒384-1211・0267-78-2121。村の「国民健康保険税について」PDF を配信。
                                 //   ★vill.minamiaiki.nagano.jp は A を返すが 403 Forbidden で村サイトを配信していない
  "www.nebamura.jp",             // 根羽村（長野県）〒395-0701・0265-49-2111・Copyright (C) NEBA Village。
                                 //   村の広報誌「広報ねば」を配信（誌面の村役場住民課の電話とトップの電話が一致）
  "www.vill-shimojo.jp",         // 下條村（長野県）〒399-2101・0260-27-2311・Copyright c 2017 SHIMOJO VILLAGE。
                                 //   村の広報誌「広報志もじょう」を配信（令和8年度の村国保税率表を掲載）
  "www.vill-tenryu.jp",          // 天龍村（長野県）〒399-1201・0260-32-2001・Copyright (C)2002-2026 Village of Tenryu。
                                 //   村の広報誌「広報天龍」を配信（令和8年度の村国保税率表を掲載）
  "www.ikedamachi.net",          // 池田町（長野県）法人番号 9000020204811 が cityCode 20481 と対応。
                                 //   〒399-8696・町の「国民健康保険税について」に令和8年度の税率表を掲載し、
                                 //   町例規集（d1-law）へのリンクも自ドメインから張っている
  // 2026-09-13 追加: 長野県5市町村の独自ドメイン。#574 で verified_r8 になったが許可ホストに無く、
  //   本番ページが ◔「一次資料照合中」表示のままだった（TASKS X154-5）。5件とも自治体系ドメイン
  //   （city/town/vill.*.nagano.jp・*.lg.jp）が名前解決せず、独自ドメインが唯一の公式サイトである（実測）。
  "www.inacity.jp",              // 伊那市（長野県）法人番号 9000020202096 が cityCode 20209 と対応。
                                 //   〒396-8617 伊那市下新田3050番地・代表電話 0265-78-4111
  "www.minamimakimura.jp",       // 南牧村（長野県南佐久郡）法人番号 3000020203050 が cityCode 20305 と対応。
                                 //   〒384-1302 南佐久郡南牧村大字海ノ口1051番地・0267-96-2211
                                 //   ★群馬県南牧村（なんもくむら・cityCode 10384）と同名。法人番号で別と確認
  "www.town-kiso.com",           // 木曽町（長野県木曽郡）法人番号の掲載は無い。
                                 //   〒397-8588 木曽郡木曽町福島2326番地6・TEL 0264-22-3000・
                                 //   copyright 木曽町 Kiso Town を照合
  "www.koumi-town.jp",           // 小海町（長野県南佐久郡）法人番号の掲載は無い。
                                 //   〒384-1192 南佐久郡小海町大字豊里57番地1・0267-92-2525 を照合。
                                 //   町の広報誌「広報こうみ」を本ホストで配信（令和8年度の町国保税率表を掲載）
  "www.hirayamura.jp",           // 平谷村（長野県下伊那郡）法人番号の掲載は無い。
                                 //   〒395-0601 下伊那郡平谷村354番地・TEL 0265-48-2211・
                                 //   copyright 平谷村役場 を照合  // 2026-09-15 追加: 沖縄（PR #578）・岐阜（#580）・熊本（#581）の R8 昇格で判明した独自ドメイン9件。
  //   いずれも道県公式の市町村リンク集から当該ホストへリンクされていることを実取得で確認し、
  //   トップページの所在地・copyright を照合した（球磨村は法人番号でも照合）。旧型ドメインはすべて名前解決しない。
  //   根拠の全文は registry/provenance-host-watch.json の basis。追加はオーナー判断（TASKS X157-3 / X159-3 / X160-3）。
  "www.ubuyama-v.jp",            // 産山村（熊本県阿蘇郡） — 熊本県「リンク集（省庁、市町村）」（pref.kumamoto.jp/soshiki/1/51179.html）から本ホストへリンク
  "www.kumamura.com",            // 球磨村（熊本県球磨郡） — 法人番号 4000020435139 が cityCode 43513 と対応（広報記事ページのフッター）
  "reihoku-kumamoto.jp",         // 苓北町（熊本県天草郡） — 熊本県「リンク集（省庁、市町村）」から本ホストへリンク
  "www.town-ono.jp",             // 大野町（岐阜県揖斐郡） — 岐阜県「岐阜県の市町村一覧」（pref.gifu.lg.jp/page/6058.html）から本ホストへリンク
  "www.kawabe-gifu.jp",          // 川辺町（岐阜県加茂郡） — 岐阜県「岐阜県の市町村一覧」から本ホストへリンク
  "www.hichiso.jp",              // 七宗町（岐阜県加茂郡） — 岐阜県「岐阜県の市町村一覧」から本ホストへリンク
  "www.chatan.jp",               // 北谷町（沖縄県中頭郡） — 沖縄県「リンク集」（pref.okinawa.jp/about/1018107.html）から本ホストへリンク
  "www.iejima.org",              // 伊江村（沖縄県国頭郡） — 沖縄県「リンク集」から本ホスト（/ieson/）へリンク
  "www.nakijin.jp",              // 今帰仁村（沖縄県国頭郡） — 沖縄県「リンク集」から本ホストへリンク
]);
