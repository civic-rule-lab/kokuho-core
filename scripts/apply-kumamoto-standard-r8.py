#!/usr/bin/env python3
# 令和8年度 熊本県 standard_r8 適用  2026-06-03
# データ: r8-values-collected-2026-05-21.md 由来(kumamoto-r8-standard-2026-06-03.json)
# 3方式(医療/後期/介護=所得+均等+平等)・子ども=所得+均等+18歳以上(平等割なし)・資産割なし
# 突合: registry prefectureSlug=="kumamoto" を cityCode で。verified_r8スキップ。欠落は2025雛形から新規。
...
PAGE = "https://www.pref.kumamoto.jp/soshiki/43/262736.html"
...
reg = json.load(open(REGISTRY)); nag = [m for m in reg["municipalities"] if m.get("prefectureSlug") == "kumamoto"]
...
def build(slug, code, name, v, createdAt, watch):
    ...
    return {"cityCode": code, "citySlug": slug, "cityName": name, "fiscalYear": 2026, "system": "kokuho", "basicDeduction": 430000,
        ...
        "meta": {"schemaVersion": "2.0", "dataVersion": "2.1.0", "status": "provisional",
            ...
            "source": {"type": "prefecture_standard", "title": "令和8年度 熊本県 市町村標準保険料率", "url": PAGE, "publishedAt": "2026"},
            ...
            "notes": f"熊本県公表の令和8年度 市町村標準保険料率を参考値として採用。法定外繰入ゼロ前提の理論値で{name}の実告示と異なる場合あり。3方式・資産割なし。子ども子育て分は18歳以上均等割(={k['perCapitaAdult18']}円)を省いたフラット暫定。市告示後 verified_r8 昇格。"},
...
print(f"{'[DRY]' if DRY else '[APPLIED]'} registry熊本 {len(nag)} / data {len(data)}")
...
print(f"合計: {len(upd)+len(crt)+len(skip)} / 45")
