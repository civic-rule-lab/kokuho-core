#!/usr/bin/env python3
# 令和8年度 青森県 standard_r8 適用  2026-06-04
...
PAGE = "https://www.pref.aomori.lg.jp/soshiki/kenko/koreihoken/kokkuho.html"
...
reg = json.load(open(REGISTRY)); miy = [m for m in reg["municipalities"] if m.get("prefectureSlug") == "aomori"]
...
def build(slug, code, name, v, createdAt, watch):
    ...
    return {"cityCode": code, "citySlug": slug, "cityName": name, "fiscalYear": 2026, "system": "kokuho", "basicDeduction": 430000,
        ...
        "meta": {"schemaVersion": "2.0", "dataVersion": "2.1.0", "status": "provisional",
            ...
            "source": {"type": "prefecture_standard", "title": "令和8年度 青森県 市町村標準保険料率", "url": PAGE, "publishedAt": "2026"},
            ...
            "notes": f"青森県公表の令和8年度 市町村標準保険料率を参考値として採用。法定外繰入ゼロ前提の理論値で{name}の実告示と異なる場合あり。3方式(所得割+均等割+平等割)・子ども分も平等割あり・資産割なし。子ども子育て分は18歳以上均等割(={k['perCapitaAdult18']}円)を省いたフラット暫定。市告示後 verified_r8 昇格。"},
...
if not DRY and pub:
    shutil.copyfile(REGISTRY, REGISTRY + ".bak8a"); json.dump(reg, open(REGISTRY, "w"), ensure_ascii=False, indent=2); open(REGISTRY, "a").write("\n")
unm = [c for c in data if c not in {m["cityCode"] for m in miy}]
print(f"{'[DRY]' if DRY else '[APPLIED]'} registry青森 {len(miy)} / data {len(data)}")
...
print(f"合計: {len(upd)+len(crt)+len(skip)} / 40")
