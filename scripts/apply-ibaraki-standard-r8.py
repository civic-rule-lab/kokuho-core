#!/usr/bin/env python3
# 令和8年度 茨城県 standard_r8 適用  2026-06-04
# データ: r8-values-collected-2026-05-21.md 由来(ibaraki-r8-standard-2026-06-04.json)
# 2方式統一(医療/後期/介護=所得割+均等割, 平等割なし=household 0)・資産割なし
# 子ども子育て分=所得割+均等割(平等割なし)・18歳以上加算はnote記載のみ(フラット均等割暫定)
# 突合: registry prefectureSlug=="ibaraki" を cityCode で。verified_r8スキップ。欠落は2025雛形から新規。
...
PAGE = "https://www.pref.ibaraki.jp/hokenfukushi/koso/kokumin/koso/guide/hokenryouritsu.html"
...
reg = json.load(open(REGISTRY)); ibr = [m for m in reg["municipalities"] if m.get("prefectureSlug") == "ibaraki"]
...
def build(slug, code, name, v, createdAt, watch):
    ...
    return {"cityCode": code, "citySlug": slug, "cityName": name, "fiscalYear": 2026, "system": "kokuho", "basicDeduction": 430000,
        ...
        "meta": {"schemaVersion": "2.0", "dataVersion": "2.1.0", "status": "provisional",
            ...
            "source": {"type": "prefecture_standard", "title": "令和8年度 茨城県 市町村標準保険料率", "url": PAGE, "publishedAt": "2026"},
            ...
            "notes": f"茨城県公表の令和8年度 市町村標準保険料率を参考値として採用。法定外繰入ゼロ前提の理論値で{name}の実告示と異なる場合あり。2方式(所得割+均等割)・平等割なし・資産割なし。子ども子育て分は18歳以上均等割(={k['perCapitaAdult18']}円)を省いたフラット暫定。市告示後 verified_r8 昇格。"},
...
if not DRY and pub:
    shutil.copyfile(REGISTRY, REGISTRY + ".bak8"); json.dump(reg, open(REGISTRY, "w"), ensure_ascii=False, indent=2); open(REGISTRY, "a").write("\n")
unm = [c for c in data if c not in {m["cityCode"] for m in ibr}]
print(f"{'[DRY]' if DRY else '[APPLIED]'} registry茨城 {len(ibr)} / data {len(data)}")
...
print(f"合計: {len(upd)+len(crt)+len(skip)} / 44")
