#!/usr/bin/env python3
# 令和8年度 鹿児島県 standard_r8 適用  2026-06-04
# データ: r8-values-collected-2026-05-21.md 由来(miyagi-r8-standard-2026-06-04.json)
# 3方式統一(医療/後期/介護=所得割+均等割+平等割)・子ども子育て分も平等割あり・資産割なし
...
PAGE = "https://www.pref.kagoshima.jp/ae02/documents/64057_20260402141120-1.pdf"
...
_raw = json.load(open(A.data))["municipalities"]   # collected key=cityCode(離島で誤り) → cityNameで再キー
data = {v["cityName"]: v for v in _raw.values()}    # ★鹿児島は cityName 突合(collected cityCodeがregistryとズレ)
assert len(data) == len(_raw), "cityName重複あり"
reg = json.load(open(REGISTRY)); miy = [m for m in reg["municipalities"] if m.get("prefectureSlug") == "kagoshima"]
...
def build(slug, code, name, v, createdAt, watch):
    ...
    return {"cityCode": code, "citySlug": slug, "cityName": name, "fiscalYear": 2026, "system": "kokuho", "basicDeduction": 430000,
        ...
        "meta": {"schemaVersion": "2.0", "dataVersion": "2.1.0", "status": "provisional",
            ...
            "source": {"type": "prefecture_standard", "title": "令和8年度 鹿児島県 市町村標準保険料率", "url": PAGE, "publishedAt": "2026"},
            ...
            "notes": f"鹿児島県公表の令和8年度 市町村標準保険料率を参考値として採用。法定外繰入ゼロ前提の理論値で{name}の実告示と異なる場合あり。3方式(所得割+均等割+平等割)・4制度すべて3方式(子ども分も平等割)・資産割なし・限度額医療66万(国基準準拠)。子ども子育て分は18歳以上均等割(={k['perCapitaAdult18']}円)を省いたフラット暫定。市告示後 verified_r8 昇格。"},
...
if not DRY and pub:
    shutil.copyfile(REGISTRY, REGISTRY + ".bak8k"); json.dump(reg, open(REGISTRY, "w"), ensure_ascii=False, indent=2); open(REGISTRY, "a").write("\n")
unm = [n for n in data if n not in {m["cityName"] for m in miy}]
print(f"{'[DRY]' if DRY else '[APPLIED]'} registry鹿児島 {len(miy)} / data {len(data)}")
print(f"更新 standard_r8: {len(upd)} / 新規: {len(crt)} -> {crt}")
print(f"スキップ verified_r8: {len(skip)} -> {skip}")
print(f"publishYear→2026: {len(pub)} / err: {err} / data未突合cityName: {unm}")
print(f"cityName不一致(警告): {namechk}")
print(f"合計: {len(upd)+len(crt)+len(skip)} / 43")
